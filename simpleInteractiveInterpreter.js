...
Interpreter.prototype.assignVariable = function (identifier, value) {
	const hasFunctionWithSameName = this.functions[identifier];
	const existingVariableValue = this.variables[value];
	if (hasFunctionWithSameName) throw new Error("Function exists");
	else if (!isNaN(+value)) {
...
Interpreter.prototype.assignFunction = function (tokens) {
	const [, identifier] = tokens;
	const hasVariableWithSameName = this.variables[identifier] !== undefined;
	if (hasVariableWithSameName) throw new Error("Used Identifier");
	const indexOfSeparator = tokens.indexOf("=>");
	const params = tokens.slice(2, indexOfSeparator);
	const body = tokens.slice(indexOfSeparator + 1).join("");
	const usedParams = body.split("").filter(item => !item.match(/[/\*\+\-=%()\d]/));
	const bodyHasUnknownVariables = [...params].sort().join("") !== usedParams.sort().join("");
	if (usedParams.length > params.length || bodyHasUnknownVariables) {
		throw new Error("Invalid identifiers in function body");
	}
	if (params.length !== new Set(params).size) throw new Error("Duplicate arguments");
	this.functions[identifier] = { params, body };
	return "";
};
Interpreter.prototype.input = function (expression) {
	...
	} else if (tokens[0] === "fn") return this.assignFunction(tokens);
	if (tokens[1] === "=" && tokens.length > 3) {
		const result = this.input(tokens.slice(2).join(" "));
		return this.input(`${tokens.slice(0, 2).join(" ")} ${result}`);
	}
	if (!expression.match(/[/\*\+\-=%]/)) {
		const [identifier, ...params] = tokens;
		if (identifier) {
			const func = this.functions[identifier];
			const nestedFunctionIndex = params.findIndex(param =>
				Object.keys(this.functions).includes(param)
			);
			if (nestedFunctionIndex >= 0) {
				const nestedFunction = this.functions[params[nestedFunctionIndex]];
				const result = this.input(
					params.splice(nestedFunctionIndex, nestedFunction.params.length + 1).join(" ")
				);
				params.push(result);
				return this.input(`${identifier} ${params.join(" ")}`);
			}
			if (params.length !== func.params.length) throw new Error("Insufficient arguments");
			let exp = func.body;
			func.params.forEach((param, i) => {
				exp = exp.replace(new RegExp(param, "g"), tokens[1 + i]);
			});
